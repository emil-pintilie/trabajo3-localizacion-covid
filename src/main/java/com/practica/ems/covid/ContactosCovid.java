...
public class ContactosCovid {
	...
	private void cerrarArchivo(FileReader fr) {
	...
	private void validarAnadirDatos(String[] datos) throws EmsInvalidTypeException, EmsInvalidNumberOfDataException, EmsDuplicatePersonException, EmsDuplicateLocationException {

		if (hayPersonaAndConstantes(datos)) {
			this.poblacion.addPersona(this.crearPersona(datos));
		}else if (datos[0].equals("PERSONA")) {
			throw new EmsInvalidTypeException("El número de datos para PERSONA es menor de 6");
		}

		if (hayLocalizacionAndConstantes(datos)){
			PosicionPersona pp = this.crearPosicionPersona(datos);
			this.localizacion.addLocalizacion(pp);
			this.listaContactos.insertarNodoTemporal(pp);
		...
		if (noHayPersonaNiLocation(datos)) {
			throw new EmsInvalidTypeException();
		}
	}
	private boolean hayPersonaAndConstantes(String[] datos) {
		return datos[0].equals("PERSONA") && datos.length == Constantes.MAX_DATOS_PERSONA;
	}
	private boolean hayLocalizacionAndConstantes(String[] datos){
		return datos[0].equals("LOCALIZACION") && datos.length == Constantes.MAX_DATOS_LOCALIZACION;
	}


	private boolean noHayPersonaNiLocation(String[] datos){
		return !datos[0].equals("PERSONA") && !datos[0].equals("LOCALIZACION");
	}
